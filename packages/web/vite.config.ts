...
function ensureApiPlugin(): PluginOption {
  return {
    ...
    async configureServer() {
      ...
      if (!runtime) {
        console.warn(
          "\x1b[33m[llm-toolkit]\x1b[0m No TS runtime found (bun, tsx). " +
          "Install one or start the API manually: pnpm --filter @llm-toolkit/api dev"
        );
        return;
      }
      ...
      while (Date.now() < deadline) {
        if (await isApiRunning()) {
          console.log(
            `\x1b[36m[llm-toolkit]\x1b[0m Started API server via ${runtime.cmd} on http://localhost:${API_PORT}`
          );
          return;
        }
      ...
      console.warn(
        "\x1b[33m[llm-toolkit]\x1b[0m API server failed to start — web UI will have no data. " +
        "Try: pnpm --filter @llm-toolkit/api dev"
      );
    },
  };
}
