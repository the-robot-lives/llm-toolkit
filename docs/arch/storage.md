...
Single SQLite file at `~/.llm-toolkit/llm-toolkit.db` (configurable via `LLM_TOOLKIT_DATA_DIR`).
...
| `LLM_TOOLKIT_WATCH_PATHS` | `~/.claude/projects` | Colon-separated JSONL source paths |
| `LLM_TOOLKIT_WATCH` | `true` | Enable/disable file watching |
